...
| **模块** | **寄存器**      | **功能描述**                    | **关键状态**             |
| ------ | ------------ | --------------------------- | -------------------- |
| **发送** | 发送数据寄存器(TDR) | 写入待发送数据，自动加载至移位寄存器，支持8/9位格式 | TXE（可写入新数据）          |
|        | 发送移位寄存器      | 按波特率将数据串行化输出，自动添加起始/停止位     | TC（发送完成标志）           |
| **接收** | 接收数据寄存器(RDR) | 存储已接收的完整数据帧，需及时读取防溢出        | RXNE（数据就绪标志）         |
|        | 接收移位寄存器      | 实时采样RX信号，自动去除帧结构并校验数据       | ORE（溢出错误） / PE（校验错误） |

```mermaid
flowchart TD
    A[初始化UART] --> B[配置发送中断]
    B --> C["使能TXE中断（USART_IT_TXE）"]
    C --> D["使能NVIC中断通道"]
    D --> E["主程序写入首字节数据至TDR"]
    E --> F{"TXE标志自动置位？"}
    F -->|Yes| G[触发TXE中断]
    G --> H[进入中断服务程序]
    H --> I["检查USART_SR寄存器TXE标志"]
    I --> J{"需要发送更多数据？"}
    J -->|Yes| K["写入新数据至TDR"]
    K --> L["自动清除TXE标志"]
    J -->|No| M["关闭TXE中断（防空触发）"]
