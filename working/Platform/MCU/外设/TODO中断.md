## STM32
### 中断优先级
> 优先级数字越小，级别越高；首先比较抢占优先级，相同时比较响应优先级

> 共四位用于设置抢占优先级和响应优先级，默认四位全部设置抢占优先级（0~15)

- **抢占优先级（Preemption Priority）**
	- 用于决定中断能否中断其他中断。抢占优先级高的中断可以抢占正在执行的抢占优先级低的中断。

- **响应优先级（Subpriority）**
	- 用于决定在同一抢占优先级下，先处理哪个中断。
```mermaid
flowchart TD
    A[中断请求] --> B[请求挂起寄存器]
    B -->| && | C{中断屏蔽寄存器}
...
- HAL_Delay()依赖于 System tick timer中断提供的时间基准。
### NVIC (嵌套向量中断控制器)
- EXTI0~EXTO4   EXTIO01234_IRQHandler
- EXTI9_5             共享EXTI9_5_IRQHandler
- EXTI15_10         共享EXTI15_10_IRQHandler
## 中断
### 中断定义
